...
def resize(im, random_crop_shape):
    rows, cols, ch = im.shape
    crop_rows, crop_cols = random_crop_shape[0], random_crop_shape[1]
    im = zoom(im, (crop_rows * 1.0 / (1.0 * rows),
                   crop_cols * 1.0 / (1.0 * cols), 1.0), order=0)
    return im


class USBreast(Dataset):
    def __init__(self, filelist, input_shape=(300, 500, 3), train=True,  transform=None):
